...
public class MyPlayerInput : MonoBehaviour
{
    ...
    private void Update()
    {
        if (IsPressLeft || IsPressRight)
        {
            ...
            var isMovable = GameManager.Instance.IsInside(GetPreviewHorizantalPosition(value));
            if (isMovable)
            {
    ...
    private List<Vector2> GetPreviewPosition()
    {
        ...
        foreach (var piece in listPiece)
        {
            ...
            position -= pivot;
            position = new Vector3(-position.y, -position.z, 0);
            position += pivot;
    ...
    private List<Vector2> GetPreviewHorizantalPosition(int value)
    {
        var result = new List<Vector2>();
        var listPiece = GameManager.Instance.Current.listPiece;
        foreach (var piece in listPiece)
